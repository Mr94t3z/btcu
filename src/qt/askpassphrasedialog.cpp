...
#include "qt/btcu/loadingdialog.h"
    ...
    case Mode::Encrypt: {
        ...
        if (ret) {
            if (newpass1 == newpass2) {
                ...
                BTCUGUI* window = static_cast<BTCUGUI*>(parentWidget());
...
{
    BTCUGUI* gui = static_cast<BTCUGUI*>(parentWidget());
...
{
    hide();
    static_cast<BTCUGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("BTCU will close now to finish the encryption process. "
               "Remember that encrypting your wallet cannot fully protect "
               "your BTCUs from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
